    ...
    this.add_course_group_to_page = function (data) {
        if (helium.data_has_err_msg(data)) {
            ...
            $("#course-group-tabs").prepend('<li><a data-toggle="tab" href="#course-group-container-' + data.id
                                            ...
                                            + (!data.shown_on_calendar ? " <i class=\"icon-eye-close\"></i>" : "")
                                            + '</span></a></li>');
...
$(document).ready(function () {
    ...
    $.when.apply(this, helium.grades.ajax_calls).done(function () {
                            ...
                            for (i = 0; i < helium.grades.courses_for_course_group[id].length; i += 1) {
                                ...
                                for (course_grade in course_grades) {
                                    if (course_grades.hasOwnProperty(course_grade)) {
                                        data.push(
                                            [new Date(course_grades[course_grade][0]), course_grades[course_grade][1]]);
                                        data_for_group += 1;
                                    }
                                }

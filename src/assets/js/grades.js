        ...
        if (helium.data_has_err_msg(data)) {
            ...
            $("div[id^='course-group-container-']").each(function () {
                let id = $(this).attr("id").split("course-group-container-")[1].split("_")[0], course_div,
                    ...
                    course_list, group_plot_details = {
                        shadowSize: 0,
                        series: {
                            lines: {show: true},
                            points: {show: true}
                    ...
                    for (i = 0; i < helium.grades.courses_for_course_group[id].length; i += 1) {
                        ...
                        for (j = 0; j < helium.grades.categories_for_course[course.id].length; j += 1) {
                            ...
                            category_table_body.append(
                                "<tr><td><span class=\"label label-sm\" style=\"background-color: " + category.color
                                + " !important\">" + category.title + "</span></td><td class=\"hidden-xs\">"
                                + category.num_homework_graded + "</td><td>" + ((parseFloat(category.weight) !== 0
                                                                                 || !course.has_weighted_grading)
                                                                                ? (parseFloat(
                                                                                                            category.overall_grade.toFixed(2)) !== -1 ? "<span class=\"badge badge-info\">"
                                + Math.round(category.overall_grade * 100) / 100 + "%" + helium.grades.get_trend_arrow(

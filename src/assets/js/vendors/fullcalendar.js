/*!
 * FullCalendar v2.0.2 (with Helium modifications)
...
(function (factory) {
    ...
    function generateShortTimeFormat(options, langData) {
        return langData.longDateFormat('LT').replace(':mm', '(:mm)').replace(/(\Wmm)$/, '($1)') // like above, but for foreign langs
...
// Called from a translation file.
    ...
    function Calendar(element, instanceOptions) {
        ...
        function _changeView(newViewName) {
            ...
            currentView = new fcViews[newViewName]($("<div class='fc-view fc-view-" + newViewName + "' />").appendTo(content), t // the calendar object
            );
        ...
        function updateTodayButton() {
            var now = t.getNow();
            if (now.isWithin(currentView.intervalStart, currentView.intervalEnd) || currentView.name == "list") {
    ...
    function EventManager(options) { // assumed to be a calendar
        ...
        var miscCopyableProps = [
            'title', 'url', 'allDay', 'className', 'editable', 'color', 'backgroundColor', 'borderColor', 'textColor'
        ];
        ...
        function mutateEvent(event, newStart, newEnd) {
            ...
            // TODO: unsure what the point of checking if allDay has changed and then clearing end, but this causes issues in Helium, so commenting out
            /*if (newAllDay != oldAllDay) {
             // if allDay has changed, always throw away the end
             clearEnd = true;
             } else */
            if (newEnd) {
                durationDelta = dayishDiff(// new duration
                    newEnd || t.getDefaultEventEnd(newAllDay, newStart || oldStart), newStart || oldStart).subtract(dayishDiff(// subtract old duration
                    oldEnd || t.getDefaultEventEnd(oldAllDay, oldStart), oldStart));
            }

            undoFunc = mutateEvents(clientEvents(event._id), // get events with this ID
                clearEnd, newAllDay, dateDelta, durationDelta);
...
// nor timezone offset when .format() is called.
    FCMoment.prototype.stripTime = function () {
        ...
        this.year(a[0]) // TODO: find a way to do this in one shot
            .month(a[1]).date(a[2]).hours(0).minutes(0).seconds(0).milliseconds(0);
...
// timezone offset when .format() is called.
    FCMoment.prototype.stripZone = function () {
        ...
        this.year(a[0]) // TODO: find a way to do this in one shot
            .month(a[1]).date(a[2]).hours(a[3]).minutes(a[4]).seconds(a[5]).milliseconds(a[6]);
...
// Break the formatting string into an array of chunks
    ...
    function ListView(element, calendar) {
        ...
        // imports
        ListViewImpl.call(t, element, calendar, 'list');
        var formatDate = calendar.formatDate;


        function incrementDate(date, delta) {
            // This is only implemented for the interface, but nothing is done here
            return date;
        }
        ...
        function render(date, delta) {
            ...
            t.table = $("<table id=\"calendar-list-table\" class=\"table table-striped table-bordered table-hover\"><thead><th></th><th>Title</th><th>Due Date</th><th>Class</th><th>Category</th><th>Materials</th><th>Priority</th><th>Grade</th><th></th></thead></table>");
            t.body = $("<tbody id=\"calendar-list-body\"></tbody>");
            $(element).html(t.table);
            var table_div = t.table.dataTable({
                lengthMenu: [
                    [50, 100, 250, 500, -1],
                    [50, 100, 250, 500, "All"]
                ],
            ...
            t.dataTable = table_div.DataTable();
            t.table.parent().find("#calendar-list-table_length").addClass("hidden-print");
            t.table.parent().find("#calendar-list-table_info").parent().parent().addClass("hidden-print");

            // Add an "Add" button to list view
            $($($("#calendar-list-table_wrapper .row")[0]).children()[1]).append("<div class=\"pull-right hidden-print\"><button id=\"create-event\" type=\"button\" class=\"btn btn-primary btn-xs\"><i class=\"icon-plus\"></i>&nbsp;Event</button> <button id=\"create-homework\" type=\"button\" class=\"btn btn-primary btn-xs\"><i class=\"icon-plus\"></i>&nbsp;Assignment</button></div>");
        ...
        function ListViewImpl(element, calendar, viewName) {
            var t = this;
                ...
                for (evt in events) {
                    ...
                    if (event) {
                        row = $(t.dataTable.row.add([event.calendar_item_type === 1 ? ("<input id=\"homework-completed-" + event.id + "\" type=\"checkbox\" class=\"ace\"" + (event.completed ? " checked=\"checked\"" : "") + " /><span class=\"lbl\" style=\"margin-top: -5px;\" />") : "",
                                                     ...
                                                     event.calendar_item_type === 1 || event.calendar_item_type === 3 ? helium.calendar.get_material_styled_titles_from_data(event.materials) : "",
    ...
    function MonthView(element, calendar) {
        ...
        function render(date) {
            $('.fc-header span.fc-button-prev').removeClass('fc-state-disabled');
    ...
    function BasicWeekView(element, calendar) { // TODO: do a WeekView mixin
        ...
        function render(date) {
            ...
            t.title = calendar.formatRange(t.start, t.end.clone().subtract(1), // make inclusive by subtracting 1 ms
                t.opt('titleFormat'), ' \u2014 ' // emphasized dash
            );
    ...
    function BasicView(element, calendar, viewName) {
        ...
        function buildCellHTML(date) { // date assumed to have stripped time
            ...
            var contentClass = tm + "-widget-content";
            var classNames = [
                'fc-day', 'fc-' + dayIDs[date.day()], contentClass
            ];
    ...
    function AgendaView(element, calendar, viewName) {
        ...
        function buildSkeleton() {
            ...
            if (opt('allDaySlot')) {
                ...
                s = "<table style='width:100%' class='fc-agenda-allday' cellspacing='0'>" + "<tr>" + "<th class='" + headerClass + " fc-agenda-axis'>" + (
                        opt('allDayHTML') || htmlEscape(opt('allDayText'))
                    ) + "</th>" + "<td>" + "<div class='fc-day-content'><div style='position:relative'/></div>" + "</td>" + "<th class='" + headerClass + " fc-agenda-gutter'>&nbsp;</th>" + "</tr>" + "</table>";
            ...
            while (slotTime < maxTime) {
                ...
                s += "<tr class='fc-slot" + slotCnt + ' ' + (!minutes ? '' : 'fc-minor') + "'>" + "<th class='fc-agenda-axis " + headerClass + "'>" + ((!slotNormal || !minutes) ? htmlEscape(formatDate(slotDate, opt('axisFormat'))) : '&nbsp;'
        ...
        function buildDayTableBodyHTML() {
            ...
            for (col = 0; col < colCnt; col++) {
                ...
                classNames = [
                    'fc-col' + col, 'fc-' + dayIDs[date.day()], contentClass
                ];
                if (date.isSame(today, 'day')) {
        ...
        coordinateGrid = new CoordinateGrid(function (rows, cols) {
            ...
            for (var i = 0; i < slotCnt * snapRatio; i++) { // adapt slot count to increased/decreased selection slot count
                rows.push([
                    constrain(slotTableTop + snapHeight * i), constrain(slotTableTop + snapHeight * (i + 1))
                ]);
            }
        });
        ...
        function computeTimeTop(time) { // time is a duration
            ...
            var top = slotTop - 1 + // because first row doesn't have a top border
            ...
            if (ev.which == 1 && opt('selectable')) { // ev.which==1 means left mouse button
                ...
                hoverListener.start(function (cell, origCell) {
                    ...
                    if (cell && cell.col == origCell.col && !getIsCellAllDay(cell)) {
                        ...
                        dates = [
                            d1, d1.clone().add(snapDuration), // calculate minutes depending on selection slot minutes
                            d2, d2.clone().add(snapDuration)
                        ].sort(dateCompare);
...
// (the left edge on a left-to-right calendar).
    ...
    function DayEventRenderer() {
        ...
        function bindDaySeg(event, eventElement, segment) {
            ...
            if (event.allDay && segment.isEnd && // only allow resizing on the final segment for an event
        ...
        function resizableDayEvent(event, element, segment) {
            ...
            handle.mousedown(function (ev) {
                ...
                function mouseup(ev) {
                    ...
                    if (dayDelta) {
                        eventResize(element[0], event, eventEnd, ev, {} // dummy jqui event
                        );

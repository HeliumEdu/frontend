...
function HeliumCalendar() {
    ...
    this.get_category_name_by_id = function (id) {
        let category_options = $("#homework-category").find("option"), name = null, option, i = 0;
    ...
    this.get_category_id_by_name = function (name) {
        let id = -1, category_options = $("#homework-category").find("option"), i = 0, option;
        for (i = 0; i < category_options.length; i += 1) {
            option = $(category_options[i]);
            if (option.html() === name) {
                id = option.val();
    ...
    this.get_calendar_item_title = function (calendar_item) {
        if (calendar_item.calendar_item_type === 1) {
            let title = (calendar_item.completed ? "<strike>" : "") + calendar_item.title + (calendar_item.completed
                                                                                             ? "</strike>" : "");
            title =
        ...
        if (localStorage.getItem("filter_show_homework") === null || localStorage.getItem("filter_show_homework")
            ...
            helium.calendar.ajax_calls.push(helium.planner_api.get_homework_by_user(function (data) {
                $.each(data, function (i, calendar_item) {
                    ...
                    const slug = helium.calendar.categories[calendar_item.category].title.replace(" ", "")
                        .toLowerCase();
                ...
                eventRender: function (event, element) {
                    ...
                    if (event.url === undefined) {
                        ...
                        course_string = event.calendar_item_type === 1
                                        ...
                                        ? ((helium.calendar.courses[event.course].website.replace(/\s/g, "").length > 0
                        ...
                        element.qtip(
                            {
                                content: {
                                    ...
                                    text: "<div class=\"row\"><div class=\"col-xs-12\"><strong>When:</strong> "
                                          ...
                                          + (event.category !== null && helium.calendar.categories[event.category].title
                                             !== "Uncategorized"
                                             ? ("<span style=\"color: "
                                          ...
                                          + (event.materials.length > 0 && helium.calendar.get_materials_from_ids(
                                            event.materials)
            ...
            helium.planner_api.get_category_names(function (data) {
                if (helium.data_has_err_msg(data)) {
                    ...
                    for (i = 0; i < data.length; i += 1) {
                        // TODO: the original intent of this was to merge like-named categories across courses for
                        //  convenient filtering, but names with emojis break this. For now, this is reverted to IDs
                        //  in slugs (meaning like-named categories won't be merged) until a tested fix can be added
                        //  for this
                        const slug = data[i].id;

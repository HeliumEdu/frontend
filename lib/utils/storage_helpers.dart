...
class PickedFileError {
  ...
  String get userMessage => switch (reason) {
    ...
    PickedFileErrorReason.wrongFileType =>
      _allowedExtensions != null && _allowedExtensions.isNotEmpty
          ? 'Please select a ${_allowedExtensions.map((e) => e.toUpperCase()).join(' or ')} file'
          : 'Invalid file type',
  };
}
...
class HeliumStorage {
  ...
  ///
  /// On web the picker enforces [allowedExtensions] natively (FileType.custom).
  ...
  static Future<PickFilesResult> pickFiles({
    ...
    for (final platFile in result.files) {
      ...
      if (allowedExtensions != null && allowedExtensions.isNotEmpty && !kIsWeb) {
        ...
        final allowed = allowedExtensions.map((e) => e.toLowerCase()).toList();

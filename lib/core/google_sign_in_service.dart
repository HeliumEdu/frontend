...
final _log = Logger('core.google_sign_in');
...
class GoogleSignInService {
  ...
  Future<String?> signInWithGoogle() async {
    try {
      _log.info('Starting Google Sign-In flow on ${kIsWeb ? "web" : "mobile"}');

      final UserCredential userCredential;

      if (kIsWeb) {
        // On web, use Firebase Auth's built-in Google Sign-In popup
        _log.info('Using Firebase Auth popup for web');
      ...
      if (firebaseIdToken == null) {
        ...
        throw HeliumException(
          message: 'Firebase authentication failed: No ID token received',
        );
      }

      _log.info('Firebase ID token obtained successfully');

      return firebaseIdToken;
    } on FirebaseAuthException catch (e) {
      // Handle Firebase Auth errors (including popup closed/cancelled on web)
      _log.warning(
      ...
      _log.severe(
        'Unexpected error during Google Sign-In - type: ${e.runtimeType}, error: $e',
        e,
        s,
      );
      ...
      throw HeliumException(message: 'Google Sign-In failed: ${e.toString()}');
    }
  }
  ...
  Future<void> signOut() async {
    try {
      ...
      _log.info('Signed out from Google and Firebase');
    } catch (e, s) {
      _log.warning('Error signing out from Google/Firebase', e, s);
    }
  }
